...
//	@Ignore
	...
	public void testNetProject_Float_ZXY() {
		testDataset(new FloatType(), new long[] {10, 100, 100}, new AxisType[] {Axes.Z, Axes.X, Axes.Y});
	}
...
//	@Ignore
	...
	public void testNetProject_Byte_XZY() {
		testDataset(new ByteType(), new long[] {100, 10, 100}, new AxisType[] {Axes.X, Axes.Z, Axes.Y});
	}
	...
	public <T extends RealType<T> & NativeType<T>> void testDataset(T type, long[] dims, AxisType[] axes) {
		...
		assertTrue("NetProject result should contain one dataset", result.size() == 1);
