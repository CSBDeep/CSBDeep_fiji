...
public class DatasetTensorBridge {
    ...
    public void setMappingInputTensorDim(final int inputTensorDim, final int mapping){
    	dimMapping[inputTensorDim+dimMapping.length-initialInputTensorShape.numDimensions()] = mapping;
    }
...
//    	System.out.println("create 5d tf array with dims: " + getDatasetDimLengthFromTFIndex(0) + " " + getDatasetDimLengthFromTFIndex(1) + " " + getDatasetDimLengthFromTFIndex(2) + " " + getDatasetDimLengthFromTFIndex(3) + " " + getDatasetDimLengthFromTFIndex(4));
    	return new float[(int) getDatasetDimLengthFromTFIndex(0)]
    					[(int) getDatasetDimLengthFromTFIndex(1)]
    					[(int) getDatasetDimLengthFromTFIndex(2)]
						[(int) getDatasetDimLengthFromTFIndex(3)]
						[(int) getDatasetDimLengthFromTFIndex(4)];
    }
...
//						+ dims[2] + " " + dims[3] + " " + dims[4]);
	 ...
	 * if a dimension is reduced by the graph, the dimension will still be present but have size 1
	 * this looks more complicated than it should, but i tried quite some simpler versions without success
	 */
...
//		printMapping();
		...
		/*
		 * iterate over possible image dimensions and check whether they exist in input dataset
		 */
...
//						System.out.println("found mapping, " + i + ": " + i + " j: " + j + " tfindex: " + findIndexNotNegative(dimMapping, j));
						 ...
						 * tfdims contains the real (existing) dimensions of the tensor shape and .. 
						 * .. should have the same length as dimMapping entries >= 0
						 */
						_dims[dataset_i] = tfdims[getIndexByIndexIgnoreNegatives(dimMapping, j)];
					}
				}
				/*
				 *  dimension exists in input dataset, but is not mapped
				 *  the output dataset should also have this dimension, but set size to 1
				 */
...
//		}
		 ...
		 * remove negative entries from _dim to match input and output dataset dimensions
		 */
